...
def test_confirmPassword_information(page: Page, before_each):
    confirmationLocator = page.get_by_text('Passwortbestätigung ist erforderlich.')
    equalLocator = page.get_by_text('Die Passwörter stimmen nicht überein.')
    expect(confirmationLocator).to_be_hidden()
    expect(equalLocator).to_be_hidden()
    page.get_by_label('Bestätige Passwort*').click()
    page.get_by_text('Bestätige Passwort*').click()
    expect(confirmationLocator).to_be_visible()
    expect(equalLocator).to_be_hidden()
    page.get_by_label('Bestätige Passwort*').fill('Tests_0!')
    page.get_by_text('Bestätige Passwort*').click()
    expect(confirmationLocator).to_be_hidden()
    expect(equalLocator).to_be_visible()
